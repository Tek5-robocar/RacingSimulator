    ...
    try:
        ...
        while True:
            ...
            if (key_map['z']['activated'] != 2 and key_map['z']['value'] != 0):
                send_command_to_unity(client, f"SET_SPEED:{key_map['z']['value']}")
            elif key_map['s']['activated'] != 2 and key_map['s']['value'] != 0:
                send_command_to_unity(client, f"SET_SPEED:{-key_map['s']['value']}")
            else:
                send_command_to_unity(client, f"SET_SPEED:0")

            if key_map['d']['activated'] != 2 and key_map['d']['value'] != 0:
                send_command_to_unity(client, f"SET_STEERING:{key_map['d']['value']}")
            elif key_map['q']['activated'] != 2 and key_map['q']['value'] != 0:
                send_command_to_unity(client, f"SET_STEERING:{-key_map['q']['value']}")
            else:
                send_command_to_unity(client, f"SET_STEERING:0")
            ...
            get_infos_raycast(client, key_map['q']['value'], key_map['d']['value'])

            if current_time - last_time >= 20:  # Check if 20 seconds have passed
                last_time = current_time
                send_command_to_unity(client, f"SET_RANDOM_POSITION")
